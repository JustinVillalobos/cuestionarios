...
$( document ).ready(function() {
    let ps = $("#preguntasValores").val();
    let psJson = JSON.parse(ps);
...
function loadQuestions(psJson){
    if(psJson.length>0){
        indicePregunta = 1;
        psJson.forEach(element => {
            addPregunta(indicePregunta,element.pregunta,element.detalles,element.ayuda,element.definiciones);
            preguntas.push({'pregunta':element.pregunta,respuestas:[],'indice':indicePregunta,detalles:element.detalles,ayuda:element.ayuda,definiciones:element.definiciones});
            indicePregunta++;
        ...
        psJson.forEach(element => {
            ...
            respuesta((indicePregunta-1),indicePregunta,element.respuesta1,0);
            respuesta((indicePregunta-1),indicePregunta,element.respuesta2,1);
            preguntas[(indicePregunta-1)].respuestas.push({id:0,respuesta:element.respuesta1});
            preguntas[(indicePregunta-1)].respuestas.push({id:1,respuesta:element.respuesta2});
            if(element.respuesta3!=""){
                respuesta((indicePregunta-1),indicePregunta,element.respuesta3,2);
                preguntas[(indicePregunta-1)].respuestas.push({id:2,respuesta:element.respuesta3});
            }
            if(element.respuesta4!=""){
                respuesta((indicePregunta-1),indicePregunta,element.respuesta4,3);
                preguntas[(indicePregunta-1)].respuestas.push({id:3,respuesta:element.respuesta4});
            }
            $("#solucion"+indicePregunta).val(element.solucion);
...
function uploadImage(cuestionario,cantidadErrores,isValid){
    ...
    if( $avatarInput[0].files.length==0){
        save($("#fileValue").val(),cuestionario,cantidadErrores,isValid);
        return;
    }
...
function save(name,cuestionario,cantidadErrores,isValid){
            ...
            if(cantidadErrores==0){
                $avatarInput = $("#avatar");
               if($avatarInput[0].files.length==0){
                cuestionario.isNotUpdate=false;
                cuestionario.imagenOld = $("#avatarValue").val();
               }else{
                cuestionario.isNotUpdate=true;
               ...
               formData.append('cuestionario',JSON.stringify(cuestionario));
                ...
                $.ajaxSetup({
                  ...
                  $.ajax({
                    type:'POST',
                    url:$("#route").val()+'/updateCuestionario',
                    data:formData,
                    processData:false,
                    contentType:false,
                    success:function(data){
